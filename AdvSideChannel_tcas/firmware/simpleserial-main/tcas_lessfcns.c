...
bool Non_Crossing_Biased_Climb(void)
{
    ...
    if (Climb_Inhibit) 
    {
        upward_preferred = Up_Separation + NOZCROSS;
    }
    else
    {
    ...
    if (upward_preferred)
    {
	result = !((Own_Tracked_Alt < Other_Tracked_Alt)) || (((Own_Tracked_Alt < Other_Tracked_Alt)) && (!(Down_Separation > Positive_RA_Alt_Thresh[Alt_Layer_Value]))); /* opertor mutation */
    }
    else
    {	
...
bool Non_Crossing_Biased_Descend(void)
{
    ...
    if (upward_preferred)
    {
	result = (Own_Tracked_Alt < Other_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= Positive_RA_Alt_Thresh[Alt_Layer_Value]);
    }
    else
    {
	result = !((Other_Tracked_Alt < Own_Tracked_Alt)) || (((Other_Tracked_Alt < Own_Tracked_Alt)) && (Up_Separation >= Positive_RA_Alt_Thresh[Alt_Layer_Value]));
    }
    return result;
}
